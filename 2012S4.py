...
def next_moves(state):
    ...
    if new_state != state and is_valid(new_state, 1):
        new_states.add(new_state)
    new_state = state[:-2]+(state[-2]+state[-1][-1:], state[-1][:-1])
    if new_state != state and is_valid(new_state, -2):
        new_states.add(new_state)
    for i in range(1, len(state)-1):
        new_state = state[:i]+(state[i][:-1], state[i+1]+state[i][-1:])+state[i+2:]
        if new_state != state and is_valid(new_state, i+1):
            new_states.add(new_state)
